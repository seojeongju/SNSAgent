...
async def generate_shorts_script(
    ...
    """유튜브 쇼츠용 한국어 대본을 생성한다."""
    from common.ais.prompts import (
        get_short_form_system_prompt,
        build_content_user_prompt,
    )

    system_prompt = get_short_form_system_prompt("youtube_shorts", "script")
    ...
    try:
        ...
        logger.error(f"Failed to generate Shorts script: {e}")
        return {"status": False, "error": str(e), "language": "ko", "platform": "youtube_shorts"}


async def generate_shorts_caption(
    topic: str,
    tone: str = "친근하고 신뢰감 있는",
    target_audience: str = "대한민국 일반 시청자",
    brand_or_product: str = "",
    ...
    """유튜브 쇼츠용 한국어 제목·설명·태그를 생성한다."""
    from common.ais.prompts import (
        get_short_form_system_prompt,
        build_content_user_prompt,
    )

    system_prompt = get_short_form_system_prompt("youtube_shorts", "caption")
    user_prompt = build_content_user_prompt(
        topic,
        platform="youtube_shorts",
    ...
    try:
        ...
        logger.error(f"Failed to generate Shorts caption: {e}")
        return {"status": False, "error": str(e), "language": "ko", "platform": "youtube_shorts"}
